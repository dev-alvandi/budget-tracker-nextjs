...
const StatsCards = ({ userSettings, from, to }: PropsTypes) => {
  const statsQuery = useQuery<GetBalanceStatsResponseType>({
    queryKey: ["overview", "stats", from, to],
    queryFn: () =>
      fetch(
        // `/api/stats/balance?from=${DateToUTCDate(from)}&to=${DateToUTCDate(to)}`
        `/api/stats/balance?from=${from}&to=${DateToUTCDate(to)}`
      ).then((res) => res.json()),
  });
...
const StatCard = ({ formatter, value, title, icon }: StatCardPropsTypes) => {
  ...
  return (
    <Card className="w-full h-24 flex items-center gap-4 p-4">
      {icon}
